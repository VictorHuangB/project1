 ...
 */
public class YangwoqizuoSingleTestActivity extends SerialPortActivity implements View.OnClickListener {
    boolean isTesting;
    boolean testOk;
    ...
    private static final int Hold_Time = 500;//800ms发送一次
...
//                YangwoqizuoSingleTestActivity.this.finish();
    ...
    private void showSameUserDialog(final UserInfo oldUserInfo, final UserInfo newUserInfo) {
        ...
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                IcSameUserDialog selectDialog = new IcSameUserDialog(YangwoqizuoSingleTestActivity.this, oldUserInfo, newUserInfo);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.yangwoqizuo_test_layout);
    ...
    protected void onDestroy() {
        ...
        if (devIdleTimer1 != null) {
            devIdleTimer1.cancel();
        }
        super.onDestroy();
    }

    CountDownTimer devIdleTimer1 = new CountDownTimer(4000, 1000) {
        @Override
        ...
        public void onFinish() {
            ...
            time_counter.setText(String.valueOf(30));
            resultCountDownTimer.cancel();
        }
    };
    ...
    class FHLHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            devIdleTimer1.cancel();
            if (!(RC_Testing.equals(code) || RC_Success.equals(code) || RC_Problem.equals(code)) && !RC_Connect_Success.equals(code)) {
                displayStatus("请打开外设");
                displayStatusTimer1.cancel();
                displayStatusTimer1.start();
                time_counter.setText(String.valueOf(30));
...
//    }
    ...
    private void startTimerCount() {
        final int seconds = Integer.parseInt(time_counter.getText().toString());
        slideSeconds = seconds;
        if (timerCounter != null) {
            timerCounter.cancel();
        }
        timerCounter = new CountDownTimer((seconds + 1) * 1000, 1000) {
            @Override
            public void onTick(long millisUntilFinished) {
                Log.d("hailong12", " onTick ");
                slideSeconds--;
                ...
                time_counter.setText(String.valueOf(slideSeconds));
            }
            ...
            public void onFinish() {
                ...
                mHandler.post(resultSuccessRunnable);
                time_counter.setText(String.valueOf(seconds));
                resultCountDownTimer.cancel();
            }
    ...
    void initView() {
        ...
        animate_view.setBackgroundResource(R.drawable.yangwoqizuo_anim);
            ...
            public void run() {
                ...
                if (!StringUtils.isEmpty(code)) {
                    ...
                    if (code.startsWith(RC_Success)) {
                        ...
                        try {
                            String lastScore = score_label.getText().toString();
...
//                }
            ...
            case R.id.time_modify:
                ...
                //弹出框修改秒数
                final TimeCounterDialog appDialog = new TimeCounterDialog(YangwoqizuoSingleTestActivity.this, "取消", "确定", true, time_counter.getText().toString());
    ...
    class LocalLoadTask extends AsyncTask<String, Void, ArrayList<UserInfo>> {
        ...
        protected ArrayList<UserInfo> doInBackground(String... params) {
            ...
            if (search != null) {
                userInfos = UserModel.queryUserInfoByCode(YangwoqizuoSingleTestActivity.this, search);
            }
            //默认按学号排序
    ...
    void handleResultSave() {
        currentResult = score_label.getText().toString();
        if (StringUtils.isEmpty(currentResult) || Integer.parseInt(currentResult) <= 0) {
            toast("无效测试成绩");
            return;
        }
        Log.d("hailong", " currentResult is " + currentResult);
        //保存最好的成绩
        if (userInfo != null) {
            UserModel.updateUserInfoInYangwoqizuo(this, userInfo, currentResult);
            toast("保存成功");
            reset();
        } else {
            ensureMp("invalid_code.ogg", tip);
